...
void executeEnumIntegrationTests(
  ...
  setUp(() async =>
      await callDB(EnumMigrator.createEnum([TestEnum.first, TestEnum.second])));
  tearDown(() async => await callDB(EnumMigrator.dropEnum("TestEnum")));

  test("1. Create Enum With Name", () async {
    await callDB(EnumMigrator.createEnum(TestEnum.values, enumName: "MyEnum"));
    await callDB(
        migrator.createTable([EnumColumn("enum_column", enumName: "MyEnum")]));
  ...
  test("4. Add Enum Value With Name", () async {
    ...
    await callDB(
        migrator.createTable([EnumColumn("enum_column", enumName: "MyEnum")]));
    await callDB(EnumMigrator.addEnumValue(TestEnum.third, enumName: "MyEnum"));
    await callDB(migrator.dropTable());
    await callDB(EnumMigrator.dropEnum("MyEnum"));
  });
  ...
  test("5. Add Enum Value If Not Exists", () async {
    await callDB(migrator
        .createTable([EnumColumn("enum_column", enumName: "TestEnum")]));
    await callDB(EnumMigrator.addEnumValue(TestEnum.third, ifNotExists: true));
    await callDB(migrator.dropTable());
  });
